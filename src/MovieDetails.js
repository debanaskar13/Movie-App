...
class MovieDetails extends Component {
    ...
    apiCall(movieId) {
        trackPromise(
            ...
            axios.get('https://api.themoviedb.org/3/movie/' + movieId + '/videos?api_key=8265bd1679663a7ea12ac168da84d2e8&language=en-US')
                .then((res) => {
            ...
            axios.get('https://api.themoviedb.org/3/movie/' + movieId + '/credits?api_key=8265bd1679663a7ea12ac168da84d2e8&language=en-US&page=1')
                .then((response) => {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (movieId !== prevProps.match.params.id) {
    ...
    render() {
        let date = new Date(this.state.details.release_date);
            ...
            <>
                        ...
                        <div className="container text-white" >
                            ...
                            <div className="row" >
                                ...
                                <div className="col-8 mt-3">
                                    <h2><b>{this.state.details.title}</b> <span style={{ color: " rgb(232, 243, 242)" }}>({releaseYear})</span></h2>
                                    <p>{releasedate}/{releaseMonth}/{releaseYear}({this.productionCountries()})&nbsp;<b style={{ fontSize: "30px" }}>.</b> &nbsp;
                                        {
                                            this.renderGenre()
                                        }
                                        <b style={{ fontSize: "30px" }}>.</b> {Math.floor(this.state.details.runtime / 60)}h {this.state.details.runtime % 60}m</p>
                ...
                <div className="container mt-5 mb-5 testimonial-group">
                    ...
                    <div className="flex_container">
                        <TopCast type='movie' id={this.props.match.params.id} />
                    </div>
                </div>
                <div className="container mb-5">
                    <div className="testimonial-group">
                        <div className="col-12">
                            <span style={{ fontSize: "1.5rem", marginRight: 9 }}>Similar to</span><Link to={"/details/" + this.props.match.params.id} style={{ textDecoration: 'none' }}><h3 style={{ display: "inline" }}>{this.state.details.title}</h3></Link>
                        </div>
                        <div className='flex_container'>
                            {
                                this.state.recommend.map((movie, index) => {
                                    return <RecommendCard type='details' name={movie.title} overview={movie.overview} path={movie.backdrop_path} id={movie.id} percent={movie.vote_average * 10} />
                                })
                            }
                        </div>
                    </div>
